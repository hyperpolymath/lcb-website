...
- **DNS:** Cloudflare (`A @ -> ${VERPEX_ORIGIN_IP}`, proxied; `CNAME www -> nuj-lcb.org.uk`, proxied)
...
1. In cPanel, confirm/add domain `nuj-lcb.org.uk` with the account docroot shown by DomainInfo.
   On this account the active docroot is `/home/${VERPEX_CPANEL_USER}/nuj-lcb.org.uk`.
...
3. Fill in the form:
   ...
   - **Document Root:** Use cPanel default for this account. Current active path is `/home/${VERPEX_CPANEL_USER}/nuj-lcb.org.uk`.
...
1. Under "Create New Database"
   - Database Name: `nujlcb_wordpress` (cPanel will prefix with username, e.g., `${VERPEX_DB_EXAMPLE_WP}`)
...
```
Database Name: ${VERPEX_DB_EXAMPLE_WP} (example - yours will differ)
Database User: ${VERPEX_DB_EXAMPLE_USER}
...
2. Click on your database name in left sidebar (`${VERPEX_DB_EXAMPLE_WP}`)
...
```php
/** Database name */
define( 'DB_NAME', '${VERPEX_DB_EXAMPLE_WP}' ); // YOUR ACTUAL DB NAME

/** Database username */
define( 'DB_USER', '${VERPEX_DB_EXAMPLE_USER}' ); // YOUR ACTUAL DB USER
...
- Verpex IP: `${VERPEX_ORIGIN_IP}`
- Cloudflare apex A: `nuj-lcb.org.uk -> ${VERPEX_ORIGIN_IP}` (proxied)
