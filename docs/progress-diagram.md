...
3. **Static/SSG + IPFS fallback** — `ddraig-ssg` content mirrors the shareable preview and is deployable via GitHub Pages or Pinata. `content/nuj-lcb-shareable-site.html` is published through `scripts/ipfs-publish.sh`, which can either update `_dnslink.ipfs.nuj-lcb.org.uk` directly or patch the Cloudflare Web3 hostname configuration for `ipfs.nuj-lcb.org.uk` when gateway mode is enabled.
4. **Planned service hostnames** — `Conference`, `Chat`, `API`, `STFP`, and `Office` are the target service hostnames. The repo includes service docs and compose material, but Cloudflare record alignment is still needed for some of them, including a naming decision between `chat` and `zulip`.
5. **Monitoring + Hypatia** — `scripts/collect-prometheus.sh` pulls telemetry from the cache, OLS, Redis, WordPress, and Cloudflare. `monitoring/exporter.ncl` converts the metric schema into Prometheus text format. Alerts from `monitoring/prometheus/alerts.yml` can feed Alertmanager, while `monitoring/alert-trigger.sh` emits incident dustfiles and can optionally dispatch the GitHub Hypatia workflow when GitHub env is configured.
