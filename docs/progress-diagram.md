...
4. **Subdomains** — All `Conference`, `Chat`, `API`, `STFP`, and `Office` CNAME/A records should point to the same Cloudflare-managed IP (or proxied host). Each service (Jitsi, Zulip, GraphQL/REST/gRPC API, secure file transfer, OnlyOffice) is described in `content/` and `docs/` for operators. `Members` area can live under `/members/` or `members.nuj-lcb.org.uk` depending on where access controls require isolation.
