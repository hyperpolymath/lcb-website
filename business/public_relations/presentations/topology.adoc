...
* Subdomains (`conference.`, `chat.`, `office.`, `api.`) live alongside the main site: Zulip/BigBlueButton/members endpoints are proxied through the same Cloudflare zone and logged via the Prometheus exporter.
...
A/AAAA, SSL, WAF" ]
  ...
  CF --> Conference["conference.nuj-lcb.org.uk (BigBlueButton)"]
  CF --> Chat["chat.nuj-lcb.org.uk (Zulip)"]
  CF --> Office["office.nuj-lcb.org.uk (Office)"]
