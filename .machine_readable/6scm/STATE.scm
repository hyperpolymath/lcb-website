  ...
  (current-position
    ...
    (working-features
      ("infra/wordpress.ctp with Chainguard wolfi-base + Dilithium5 signature spec"
       ...
       "Cloudflare DNS cutover complete (A apex -> ${VERPEX_ORIGIN_IP}, proxied CNAME www -> apex)"
    ...
    ((date "2026-02-17")
     (accomplishments
       ("Configured Cloudflare zone + DNS cutover for nuj-lcb.org.uk"
        "Set apex A to ${VERPEX_ORIGIN_IP} and proxied www CNAME to apex"
        ...
        "Created dedicated WordPress database/user (${VERPEX_DB_MAIN} / ${VERPEX_DB_USER})"
